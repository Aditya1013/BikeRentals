...
univariate_numeric(day$casual)

# analyse the distrubution of  independence variable 'casual'
univariate_numeric(day$regestered)
...
ggplot(day, aes(x=as.factor(mnth), y=cnt),fill="grey") + 
...
ggpairs(day[,c('atemp','temp','hum','windspeed','cnt')])
...
rel_mnth_holi= table(day$season,day$holiday)
...
rels_cats_2 <- table(day$season,day$weekday)
rels_cats_2
barplot(rels_cats_2)

#check relationship between  season and weathersit

rels_cats_3 <- table(day$weathersit,day$season)
rels_cats_3
...
rels_cats_4 <- table(day$weathersit,day$holiday)
rels_cats_4

barplot(day$weathersit,day$holiday)
...
val = day_out$casual[day_out$casual %in% boxplot.stats(day_out$casual)$out]

 day_out = day_out[which(!day_out$casual %in% val),]

 # Boxplot after removing  outliers
 
 # boxplot for  casual variable
 
 ggplot(data = day_out, aes(x = "", y = casual)) + 
   geom_boxplot() 
 
 
# verify the relationship after  outliers
 ggplot(day_out, aes(x= casual,y=cnt)) +
   geom_point()+
   geom_smooth()

cor(day$casual,day$cnt)
cor(day_out$casual,day_out$cnt) 
...
library(corrgram)
corrgram(day[,c('temp','atemp','hum','windspeed','cnt')], order = F,
...
day_features = subset(day,select=-c(atemp,hum))
...
for(i in cnames){
  print(i)
  day_features[,i] = (day_features[,i] - min(day_features[,i]))/
    (max(day_features[,i] - min(day_features[,i])))
}
...
set.seed(1234)
library(caret)
train.index = createDataPartition(day_features$cnt, p = .80, list = FALSE)
train = day_features[ train.index,]
test  = day_features[-train.index,]
